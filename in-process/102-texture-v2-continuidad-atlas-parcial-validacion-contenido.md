...
- **Verificación en vivo: PENDIENTE al momento de abrir el PR** — la API real nunca se llama desde la suite automatizada (`ai.image-provider=mock`), así que el formato `image[]` solo puede confirmarse contra `studio-dev` DESPUÉS del deploy de este ticket. El Javadoc de `callEdits` dice exactamente eso, sin afirmar una verificación que todavía no ocurrió (precedente 059/060/061/063: la documentación oficial sola ya falló tres veces contra la API real). El resultado concreto se registra abajo antes de cerrar el ticket.
