...
import { cancelGeneration, eventsUrl, startGeneration, type GeometryDetail } from '../../api/generationApi'
...
const props = defineProps<{ mobId: string; projectId: string; mobName: string; baseType: BaseType; geometryDetail: GeometryDetail }>()
...
async function beginGeneration(): Promise<void> {
  ...
  try {
    const response = await startGeneration(props.mobId, props.geometryDetail)
