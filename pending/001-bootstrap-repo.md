...
- Dado el análisis de SonarQube **dentro del pipeline de Jenkins** (`withSonarQubeEnv('sonarqube-vm')` en el `Jenkinsfile`, credential ya configurado en Jenkins — mismo patrón que `auth-core-mc`/`mail-core-mc`), cuando corre un build real, entonces el análisis se envía y el Quality Gate se evalúa — esto NO depende de `SONARQUBE_CLI_TOKEN_VM`.
- Dado `SONARQUBE_CLI_TOKEN_VM` en `~/dev-infra/.env` (uso personal de Marco desde su Mac vía `~/dev-infra/scripts/sonar-vm.sh`, un mecanismo aparte del anterior), cuando está vacío, entonces ese uso puntual queda marcado como bloqueado — no bloquea el pipeline de CI.
