...
- Body opcional (ticket 109, **reemplaza al `textureResolution` del 103**): `{"textureDensity": "standard"|"high"|"max"}` — densidad de téxel: cuántos téxeles recibe cada unidad de modelo (`standard`=1, `high`=2, `max`=4). Ausente/null usa `max`. El tamaño del atlas es **consecuencia del packing a esa densidad, sin tope** (Diseño técnico §7). **Cambio de contrato**: el campo `textureResolution` (tope de atlas) ya no existe — se eliminó porque, medido contra un mob real de 46 cuboides, el tope degradaba la densidad justo en los modelos complejos (su atlas ya superaba el tope a la densidad más baja), y además la densidad nunca llegaba al packing: solo dimensionaba el lienzo. Ver `docs/definiciones/densidad-de-texel-y-resolucion-de-textura.md`.
