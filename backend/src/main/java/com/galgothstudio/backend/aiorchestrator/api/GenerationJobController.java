...
public class GenerationJobController {
	...
	public ResponseEntity<StartGenerationResponse> start(@PathVariable UUID mobId, @RequestBody(required = false) StartGenerationRequest body) {
		...
		TextureDensity textureDensity = body != null && body.textureDensity() != null ? body.textureDensity() : TextureDensity.MAX;
		UUID jobId = mobGenerationService.startGeneration(mobId, geometryDetail, textureDensity);
