...
- **Verificado en vivo de punta a punta contra `studio-dev.galgoth.64bitstudio.com` real** (post-deploy): registro real (`Ada Ticket078`, correo de verificación real disparado — "Revisa tu correo"), login real con esas credenciales → redirige a "/" (Inicio), `sessionStorage` con el `user`/tokens reales confirmado vía JS en la pestaña. Usuario de prueba eliminado en auth-core-mc DEV al terminar.
