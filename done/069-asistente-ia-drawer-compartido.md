...
**Verificación en vivo contra `studio-dev`** (post-merge, PR #91, commit `ca8fba9`, redeploy confirmado `StartedAt: 2026-09-11T07:11:27Z`): el mismo `GDrawer` abre correctamente en las dos tabs -- Modelo muestra "Asistente IA" con el contenido intacto de `AiEditPanel` (textarea + "Generar cambios"), Textura muestra "Generador de textura (IA)" con el rediseño de 1 sola columna (referencia real + overlay "Cambiar imagen", 4 tarjetas de Estilo, segmentado de Detalle, "Parte a generar", botón "Generar con IA"), sin overflow horizontal ni scroll lateral, cerrando limpio con el botón ×. Sin hallazgos.

**Hallazgo de entorno (no de producto), documentado en memoria `claude-in-chrome-frozen-css-transition-gotcha`**: un screenshot tomado inmediatamente tras el clic que abre el drawer, en el tab automatizado de Claude-in-Chrome, puede capturar la transición CSS congelada a medio camino (throttling del tab no enfocado) y aparentar overflow -- confirmado que es un artefacto del entorno de automatización forzando `transition:none` vía JS (resuelve al layout correcto al instante), no un bug real del componente.
