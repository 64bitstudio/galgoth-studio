...
**Cierre**: mismo criterio que tickets 071/072 -- PO autorizó el flujo git normal, consolidado en el PR #97 (`feat/071-076-rediseno-ui-inicio-editor-wizard`), mergeado a `dev` con CI verde y ambos Quality Gates de SonarQube (backend/frontend) verificados `OK` por SQL.

**Hallazgos reales de SonarQube (Quality Gate, post-PR)**: `RenameProjectRequest.java` y `ProjectControllerTest.java` tenían la palabra "todo caller" (español, "cualquier caller") disparando el falso positivo de S1135 ("TODO comment") -- reescrito sin cambiar el sentido. `ProjectControllerTest.java` tenía además una variable local asignada pero nunca leída (S1854/S1481) -- reemplazada por una aserción real (`andExpect(status().isCreated())`). En `ProjectDetail.vue` (docstring del componente, línea con la cita textual del PO: *"agrega transiciones... todo debe llevar transiciones"*), la misma palabra "todo" disparaba S1135 -- decisión: NO alterar la cita textual del PO (queda intacta arriba, en la sección "Decisión del Product Owner"), se parafraseó en cambio el texto DEL DOCSTRING que la rodeaba en el código, referenciando este archivo en vez de re-citar. `ConfigurationStep.vue` (ticket 074) tenía un `<label>` envolviendo un `GSelect` -- asociación inválida para lectores de pantalla (S6853, `GSelect` no es un control nativo) -- cambiado a `<div>` sin efecto visual.
