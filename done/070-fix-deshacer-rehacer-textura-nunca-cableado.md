...
**Verificación en vivo contra `studio-dev`** (post-merge de ambos PRs, #93 y #94, commit final `301b2aa`, redeploy confirmado `StartedAt: 2026-09-11T16:22:27Z`): en Textura, pintar un píxel habilita "Deshacer"; clic en "Deshacer" borra el píxel del `<canvas>` visible (tooltip "Deshacer Ctrl+Z"); clic en "Rehacer" lo vuelve a pintar (tooltip "Rehacer Cmd+Shift+Z"). Ambos hallazgos (botón nunca cableado + canvas sin redibujar) confirmados corregidos de punta a punta en producción, no solo en tests.
